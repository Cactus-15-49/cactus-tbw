...
export class Pay {
    ...
    public async pay(): Promise<void> {
        const db: Database = new Database(this.configuration.get("dbPath") as string);
        let newHeight = 0;
        ...
        for (const block of forgedBlocks) {
            ...
            newHeight = block.height > newHeight ? block.height : newHeight;
            ...
            const wallets = voters
                ...
                .map((w) => {
                    if (fidelity) {
                        const balances = db.getAddressBalancesForNPreviousBlocks(w.address, block.height, fidelity);
        ...
        const fullTransferFee = this.calculateFees(transfersMax, memo, !!pp[1], extraFee);

        const lastTransferFee =
            transfersInLastTransfer > 0
                ? this.calculateFees(transfersInLastTransfer, memo, !!pp[1], extraFee)
                : fullTransferFee;
        ...
        for (const chunk of txChunks) {
            ...
            db.addHistory(newHeight, transaction, username);
        }
    ...
    public async replay(id: string): Promise<{ success: boolean; error?: string }> {
        ...
        db.addHistory(databaseTx.height, transaction, wallet.getAttribute<string>("delegate.username"));
        db.setTransactionToRepaid(id);
    ...
    private async sendTransaction(transactions: Interfaces.ITransactionData[]) {
        ...
        if (errors) {
            for (const error of Object.keys(errors)) {
                ...
                db.setTransactionToError(error);
